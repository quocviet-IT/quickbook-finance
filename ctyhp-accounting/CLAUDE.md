...
- UI change: also run `scripts/smoke-pages.mjs`. The four gates above all pass on a page that throws at render time. It needs **no** `ALLOW_DESTRUCTIVE_E2E` — it signs in once and issues a GET per route, writing nothing, so it runs against whatever project `.env.local` points at. Sign-in comes from `scripts/smoke-environment.mjs`, which mints a one-time link with the service role key already in the environment; set `SMOKE_EMAIL`/`SMOKE_PASSWORD` to be explicit instead. Run it **against the built server, never `npm run dev`**:
  ```
  npm run build          # ~47s
...
- A settings screen's audience is declared once, on its `SETTINGS_HUB` entry in `lib/domain/navigation.ts`. The hub filters with `settingsHubForAccess`, and each page calls `requireSettingsAccess("<its own href>")`, which reads the same entry — so a card and its door cannot disagree, and a new settings page with no catalog entry makes its guard throw rather than open. `/settings` and `/settings/feedback` are the two deliberate exceptions, named in `tests/unit/navigation.test.ts`. Note that a page-level `redirect()` arrives as a **200 with `<meta http-equiv="refresh">`**, not a 307: the app shell is an async layout, so the response has already begun streaming. It is still a refusal — the page body never renders — and `scripts/verify-settings-guard.mjs` asserts exactly that, signing in as a real non-administrator. Only a redirect issued from a layout gets a clean 307.
- `feedback.read` is admin-only since migration 0099. `acc_feedback_queue` is `security definer` and filters on that permission alone, so it returns **nothing** to anyone else rather than their own rows — a reporter's view must read `acc_feedback_report` directly and let RLS narrow it (`… or reporter_id = auth.uid()`). Never wire `MyReportsClient` to the queue RPC. The two feedback *storage* policies are global objects pinned to `public.`, so outside `public` their owner fallback consults the wrong table — a known multi-company gap that predates 0099, not something 0099 introduced.
- A gate must name what the server actually enforces. A gate looser than the mutation behind it is a hole; a gate tighter than it is a false statement that invites someone to loosen the mutation to match. Two gates were corrected on exactly this ground: Accounting periods is `roles: ["admin"]` because every action calls `adminGuard`, and Import is `roles: ["admin", "accountant"]` because its actions guard with `canWrite` for everything but the chart of accounts.
...
- Every HTTPS end-to-end suite requires `ALLOW_DESTRUCTIVE_E2E=ONEBOOK_TEST_DATABASE_ONLY` **and** a second Supabase project — `requireDestructiveE2eEnvironment` refuses `E2E_*` values that resolve to the same host as the live ones, so the flag alone opens nothing. Those suites write real documents and cannot run against production, by design. Some checks carry their own guarantee instead and are safe anywhere: `tests/e2e/company-isolation.e2e.ts` refuses to write outside a company marked `is_sample`, and the `scripts/verify-*.mjs` harnesses that authenticate through `request.jwt.claims` — `verify-settle-from-bank.mjs`, `verify-fixed-assets.mjs` — run every scenario inside a transaction they roll back. That pattern is how to prove an RPC *posts* rather than merely exists; the first one written caught a settlement function that had passed every structural check and could not resolve its own party. `scripts/smoke-pages.mjs` is not gated at all any more — it only reads (see §2).
