...
import { createCustomer, updateCustomer, InvoicingError } from "@/lib/services/invoicing";
import { customerCreateSchema, customerUpdateSchema } from "@/lib/domain/schemas";
...
export async function updateCustomerAction(raw: unknown): Promise<ActionResult> {
  ...
  try {
    const sb = await createSupabaseServerClient();
    await updateCustomer(sb, parsed.data);
    revalidatePath("/customers");
    revalidatePath("/invoices");
    return { ok: true };
  } catch (err) {
