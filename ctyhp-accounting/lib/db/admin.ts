 ...
 * inviting a user through the Auth admin API, which no anon-key client can do.
 ...
 * before offering to invite anyone, rather than letting the Auth API fail with
...
export function createSupabaseAdminClient(): SupabaseClient {
  ...
  if (!isAdminClientConfigured()) {
    throw new Error(
      "Inviting users needs a real SUPABASE_SERVICE_ROLE_KEY in the environment; " +
