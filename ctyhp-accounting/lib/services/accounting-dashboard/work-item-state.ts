...
import type { WorkItemState, WorkLifecycle } from "@/lib/domain/accounting-dashboard/lifecycle";
 ...
 *
 * Owner names are resolved through `acc_actor_directory` rather than joined:
 * the table references `auth.users`, which an application session cannot read,
 * and a uuid on screen tells nobody who is holding the work.
 */
 ...
 *
 * Called with the keys the queue just produced, at the one moment the live set
 * is actually known. A failure here costs the tidy-up, never the queue — so it
