 ...
 *
 * `p_expected_updated_at` is the concurrency token: the caller sends back the
 * timestamp it rendered, and a row that has moved since is refused rather than
 ...
 *
 * Null `p_expected_updated_at` means "I believe there is no row yet".
 */
...
begin
  ...
  if v_current.work_key is null then
    ...
    insert into acc_work_item_state (
      work_key, lifecycle, owner_id, due_date, dismiss_reason, updated_at, updated_by
    ) values (
      btrim(p_key), p_lifecycle, p_owner_id, p_due_date, v_reason, v_now, auth.uid()
    );
  else
...
revoke all on function acc_set_work_item_state(text, text, uuid, date, text, timestamptz, boolean)
  from public, anon;
grant execute on function acc_set_work_item_state(text, text, uuid, date, text, timestamptz, boolean)
