-- ============================================================================
-- 0105  Which accounts a transaction touched
--
-- Asked for on /reports/transactions, 2026-08-06: "Can we add a filter? Like
-- filter it by vendor, filter it by account".
--
-- Filtering by account cannot use the labels this report already returns. A
-- side of an entry that touches more than one account is labelled "— Split —",
-- so a reviewer looking for everything that hit one account would get a list
-- with every split entry missing from it — a filter that hides matching
-- transactions, which is worse than no filter because it is believed.
--
-- The function therefore returns the accounts each entry actually touched, and
-- the screen matches against those. Ids rather than names: this chart holds
-- two accounts called "Cash on Hand", and names would merge them.
--
-- Nothing else changes — same parameters, same rows, same order.
-- ============================================================================

set search_path = public;

-- Adding a column to the returned table changes the function's return type,
-- which `create or replace` refuses. Dropping first is safe here: the grants
-- are reissued at the foot of this file, and nothing holds a reference to the
-- function except the report that calls it by name.
drop function if exists acc_transaction_list(date, date);
...
language sql stable security definer set search_path = public as $$
  ...
  select
    ...
    -- Every account this entry touched, so the screen can filter on an account
    -- and still find the entry when its label reads "— Split —".
    coalesce(
      (select array_agg(distinct l.account_id)
         from acc_journal_line l
        where l.journal_entry_id = e.id),
      array[]::uuid[]
    )
